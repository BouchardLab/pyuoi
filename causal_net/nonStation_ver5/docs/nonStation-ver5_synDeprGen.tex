...
($\Delta t = 1$\,ms), the probability of two spikes per bin is of order
...
the model: presynaptic spikes update the STD state, the resulting release is
filtered by the lag kernel, and the recurrent weighted input determines the
postsynaptic spike probability.
...
\midrule
Time bin width         & $\Delta t$            & $1$\,ms                           & Resolves individual spikes; ensures $p\ll 1$ \\
...
Utilization parameter  & $U$                   & $0.3$--$0.5$                      & Moderate depression, common for all neurons \\
Weight scale           & $v, R$                & $v=0.4,\ R=0.90$                  & Uniform weights scaled by spectral radius \\
Baseline log-odds      & $b_i$                 & tuned per neuron                  & Yields $30$--$50$\,Hz firing \\
