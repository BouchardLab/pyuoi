...
def edge_selector_fdr(A_edges_real_list, A_edges_shuf_list, alpha=0.01):
    ...
    E_pval = np.ones((N, N))
    E_edge_mask = np.zeros((N, N), dtype=bool)
    #E_edge_mask is a binary mask of significant edges after row-wise FDR.
    ...
    for i in range(N):
        ...
        reject, _ = fdrcorrection(E_pval[i, mask], alpha=alpha)
        E_edge_mask[i, mask] = reject
    ...
    selected_edges = int(E_edge_mask.sum()) - N  # Subtract diagonal (always True)
    sparsity_E_mask = 1.0 - (selected_edges / off_diag_elements)
    ...
    summary = {
        ...
        "matrix_size": N,
        "num_significant_edges": int(E_edge_mask.sum()) - N,  # Exclude diagonal
        ...
        "sparsity_E_mask": float(sparsity_E_mask),
    ...
    return E_edge_mask, E_pval, summary
