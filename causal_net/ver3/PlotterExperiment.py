...
def summary_column(md):
    ...
    pmd=md['dataset']
    txt=md['short_name']
    dt=pmd['step_duration']
    txt+='\nbin size %d msec'%(dt*1000)
    txt+='\nsampFreq %d Hz'%(1/dt)
...
#...!...!..................
    def fano_and_cov_fit(self, expD, expMD, figId=5):
        #pprint(expMD)
        tit = 'dataset: ' + expMD['short_name']
                
        figId=self.smart_append(figId)        
        nrow,ncol=1,3
        fig=self.plt.figure(figId,facecolor='white', figsize=(11,4))
        ...
        #  Top plot: Fano factor 
        ax = self.plt.subplot(nrow,ncol,1)
        ax.set_title(tit)
        #ax.set_title(tit)
        
        plot_fano(ax, spike_moments, expMD)

        #.....  auto-correlation
        ax = self.plt.subplot(nrow,ncol,2)
        plot_cov_fit(ax, expD,expMD,tag='auto_cov')
        
        #.....  cross-correlation
        ax = self.plt.subplot(nrow,ncol,3)
        plot_cov_fit(ax, expD,expMD,tag='cross_cov')
        
# - - - - - -   CLASS end - - - - - --

#...!...!...................
def plot_fano(ax1, spike_moments, md):
    ...
    #sampling_freq = md['dataset']['sampling_freq']  # Hz
    
    ## Convert to spike rate (spikes/sec)
    window_duration_sec = md['dataset']['step_duration'] *window_size_bins #/ sampling_freq
    avg_spike_rate = mean_spike_count / window_duration_sec
        
    
    txt=summary_column(md)
    txt+='\n avg_spike_rate:%.1f Hz'%avg_spike_rate
    ax1.text(0.02, 0.95, txt, fontsize=10, color='blue', ha='left', va='top',transform=ax1.transAxes)

    ax1.legend(loc=(0.45,0.1))
    ax1.grid(True)

#...!...!...................
def plot_cov_fit(ax2, bigD, md, tag):
    dsd=md['dataset']
    stepSize=dsd['step_duration']
    covV=bigD[tag+'_data']
    nT=covV.shape[0]
    timeV=np.arange(nT,dtype=np.float32)*stepSize
    
    # plot empirical
    ioff=10
    data_shape = md['cov_spikes_data_shape']
    dLab=f'data: {data_shape[0]}f  × {data_shape[1]}t '
    ax2.plot(timeV[ioff:], covV[ioff:], 'k.', label=dLab)  # skip 0-time
    ...
    A = fit_params['A']
    B = fit_params['B']
    start_idx = fit_params['fit_start']
  
    # data for fit‐line
    t_fit = timeV[start_idx:]
