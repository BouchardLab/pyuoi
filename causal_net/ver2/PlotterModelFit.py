...
from Plotter_Dale_LDS import plot_dale_matrix, plot_dale_eigen
...
#...!...!..................
    def W_and_eigen(self,bigD,md,tf, figId=3):
        ...
        if tf==True:  # Dale-truth
            ...
            W=np.copy(A0).T
            tit='Dale true, M%d, %s'%(A0.shape[0],md['selector']['input_name'])
            tit2='True eigen values'
        else: # UoI fit
            A0=bigD['fit_A_model'][0].T
            W=A0-np.eye(A0.shape[0])
            tit='Fit true, M%d, %s'%(A0.shape[0],md['selector']['input_name'])
