...
def print_evaluation_report(stats, args, used_sparsity):
    print(f"\n--- Evaluation vs. Ground Truth (E) for '{args.inpName}' after {args.num_epochs} epochs, prob_thresh={args.edge_probability:.3f}, actual_sparsity={used_sparsity:.3f} ---")
...
def report_results(W_est, W_confidence, truth_E, mask, train_stats, args, device, used_sparsity,num_sampl):
    W_est_np = W_est.cpu().numpy().astype(int)
    ...
    if args.verb > 0:
        ...
        print(f"Fitting took {train_stats['elapsed_minutes']:.2f} minutes, input size:{num_sampl}",)
        if device.type == 'cuda':
