...
def create_evaluation_plot(W_true, W_fitted, losses, fit_time, args, num_samples_k, num_excite, num_inhibit):
    ...
    fig = plt.gcf()
    num_epochs = len(losses)
    fig.suptitle(f'Fit for {args.input}, trained on {num_samples_k:.0f}k samples for {num_epochs} epochs, took {fit_time:.1f} sec', fontsize=16)
