...
\title{PRISM-EM for Non-Stationary Poisson GLMs from Spike Trains:\\
...
\begin{abstract}
This document describes the current \texttt{prism\_EM\_train.py} implementation for fitting a non-stationary Poisson GLM with latent state mixing (PRISM-EM)~\footnote{PRISM: PRobabilistic Inference of Sparse Mixed-state dynamics; ~~EM: Expectations Maximization}. The model uses a shared connectivity matrix across states, state-specific bias vectors, and time-varying simplex coefficients. Training is performed by block coordinate descent: a sequential projected-gradient E-step for coefficients followed by Viterbi decoding to one-hot state assignments, and an Adam-based M-step for model parameters with off-diagonal L1 shrinkage and spectral-radius projection. Spectral-radius enforcement, L1 pruning, and learning-rate decay are each activated after a configurable number of EM iterations. We summarize the exact objective used in code, the decoding method, all current default hyperparameters, and initialization modes (data-driven vs random) for \(A\), \(B\), and \(C\).
\end{abstract}
...
% ---------------------------------------------------------------
\section{EM Training for Non-Stationary Poisson GLM}
\label{sec:em}
% ---------------------------------------------------------------
...
Viterbi decoding is applied both within the EM loop (after each E-step, to produce one-hot assignments for the M-step) and after the final EM iteration (to produce the output state sequence). The decoder uses a homogeneous transition prior:
\begin{equation}
...
M-step & \texttt{--lr\_mstep} & \(3 \times 10^{-3}\) \\
 ...
 & \texttt{--lambda3} & 0.02 \\
 & \texttt{--rho\_max} & 0.95 \\
 & \texttt{--prescale\_m\_step\_4\_ArhoMax} & 50 (batches) \\
 ...
 & \texttt{--minW} & 0.01 (reporting only)\\
...
\bottomrule
\end{tabular}
\caption{Current defaults in \texttt{prism\_EM\_train.py}.}
\end{table}

\paragraph{Required input.}
\texttt{--num\_states} is required (no default).
...
\begin{enumerate}
  ...
  \(\lceil\tau_{\mathrm{dwell}}/\Delta t\rceil\) bins.
