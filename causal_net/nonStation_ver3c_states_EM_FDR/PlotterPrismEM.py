...
class Plotter(PlotterBackbone):
    ...
    def _draw_A_matrix(
        self, ax, A, title, num_exc=None, norm_map=None,
        exc_inh_from_offdiag=False, exc_inh_min_abs=0.0,
    ):
        A = np.asarray(A)
        if norm_map is None:
            vmin = float(np.min(A))
    ...
    def _overlay_single_rates(self, ax, single_rates, x_post):
        ...
        ax2 = ax.twinx()
        ax2.plot(x_post, rate, color='tab:orange', linewidth=0.9, alpha=0.75)
    ...
    def A_fitted_prismEM(self, fitD, md, single_rates, figId=8):
        ...
        for row, (A, label, diag_txt) in enumerate([
    ...
    def compare_A_vs_truth(self, A_cmp, md, cmp_label='A_init', figId=3):
        ...
        txt = self._A_init_diagnostics_txt(md)
        ax.text(
    ...
    def node_outgoing_edge_stats_prismEM(
        ...
        def add_hist_percentile_marker(ax, vals, fmt=".3f"):
            ...
            p16_txt = format(p16, fmt)
            p50_txt = format(p50, fmt)
            p84_txt = format(p84, fmt)
            txt = f"p16={p16_txt}\nmed={p50_txt}\np84={p84_txt}"
            ax.text(
        ...
        for cls, color, label in [
            ...
            if cls == 0:
                ...
                ax.scatter(
                    Nedge[mask], Sedge[mask], s=16, marker='.',
    ...
    def _state_seq_time_slice(self, trainMD, time_range_sec):
        """Return index slice [i0:i1+1] and time axis for --time_range_sec."""
        dt = float(trainMD["time_step_sec"])
        t0_bin, t1_bin = [int(x) for x in trainMD["time_range_bins"]]
        t0_req, t1_req = [float(x) for x in time_range_sec]
        if t1_req < t0_req:
    ...
    def _draw_state_c_panel(
        self, ax, t_bins, S, S_cl, C, title,
        ...
        ax.set_xlim(float(t_bins[0]), float(t_bins[-1]))
        ax.yaxis.set_major_locator(MaxNLocator(integer=True))
        ax.grid(True, alpha=0.3)
    ...
    def state_seq_fitonly_prismEM(
        self, fitD, md, rebD, bio_plot_md, figId=9, time_range_sec=None,
    ):
        ...
        C_init = np.asarray(fitD["c_init"], dtype=np.float64)[i0:i1 + 1]
        S_init_cl = 1.0 - np.max(C_init, axis=1)

        S_hat = np.asarray(fitD["S_hat"], dtype=np.float64)[i0:i1 + 1]
        C_hat = np.asarray(fitD["c_hat"], dtype=np.float64)[i0:i1 + 1]
        S_hat_cl = np.asarray(fitD["S_hat_CL"], dtype=np.float64)[i0:i1 + 1]

        clip = clip_rebD_time(rebD, time_range_sec)
        dataset_name = bio_plot_md["short_name"]

        figId = self.smart_append(figId)
        fig = self.plt.figure(figId, facecolor="white", figsize=(12, 14))
        gs = fig.add_gridspec(5, 1, height_ratios=[0.14, 0.14, 0.12, 0.50, 0.01])

        ax = fig.add_subplot(gs[0, 0])
        self._draw_state_c_panel(
            ax, t_bins, S_init, S_init_cl, C_init, "Initialization",
            s_label="S_init", cl_label="S_init_CL", c_ylabel="C_init",
            show_xlabel=False,
        )

        ax = fig.add_subplot(gs[1, 0])
        self._draw_state_c_panel(
            ax, t_bins, S_hat, S_hat_cl, C_hat, "Fit",
            s_label="S_hat", cl_label="S_hat_CL", c_ylabel="C_hat",
            show_xlabel=False,
        )
        ...
        PlotterBioExp.plot_bioexp_heatmap(self, fig, ax, rebD, dataset_name, clip)

        t0s, t1s = float(time_range_sec[0]), float(time_range_sec[1])
        fig.suptitle(
            f"State sequences: {md['short_name']}  T=[{t0s:g}, {t1s:g}] s",
            fontsize=12,
        )
        fig.tight_layout(rect=[0.0, 0.0, 1.0, 0.94])
        ...
        if sign is None:
            ...
            for i_post, j_pre in zip(post_draw, pre_draw):
                ax.plot(
                    [loc_x[j_pre], loc_x[i_post]],
                    [loc_y[j_pre], loc_y[i_post]],
                    color=edge_color, alpha=0.7, linewidth=0.7, zorder=2,
                )
        ax.set_aspect("equal", adjustable="box")
    ...
    def neuron_spatial_Ahat_edges(self, fitD, bioD, bioMD, figId=10):
        ...
        rate = PlotterBioExp._metrics_column(self, bioD, bioMD, "firing_rate")
        A_hat = np.asarray(fitD["A_hat"], dtype=np.float64)
        n_neur = loc_x.shape[0]
        assert A_hat.shape == (n_neur, n_neur), (
            f"A_hat shape {A_hat.shape} does not match {n_neur} neuron locations"
        )

        figId = self.smart_append(figId)
        fig = self.plt.figure(figId, facecolor="white", figsize=(15, 10))
        gs = fig.add_gridspec(1, 3, width_ratios=[1.0, 0.06, 1.0], wspace=0.28)
        ...
        ax.tick_params(labelsize=9)
        ax.grid(True, alpha=0.35)
        ax.set_title(f"dataset: {bioMD['short_name']}   n={n_neur} neurons", fontsize=11, pad=8)
        ...
        fig.suptitle(f"Neuron spatial A_hat topology: {bioMD['short_name']}", fontsize=12)
        fig.tight_layout(rect=[0.0, 0.0, 1.0, 0.94])
