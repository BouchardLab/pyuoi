...
\hat{\mathbf B}^{\text{init}},\,\theta_{\text{M}}\,)
...
one-hot assignment, with no E-step and no Viterbi re-decoding), and returns
the re-estimated connectivity \(A\) and the refit state biases
\(\mathbf B\). The supplied \(\hat{\mathbf B}^{\text{init}}\) is the
time-ordered fit's \(\hat{\mathbf B}\); it initializes the locked M-step but
is not frozen. Re-fitting \(\mathbf B\) in the scrambled context lets the
null absorb state-specific baseline effects, so the test of \(A\) is not
made artificially harsh by a bias mismatch.
...
\(\hat S^{\text{lock}}_{t-1}\) for pair \((Y_{t-1},Y_t)\). This
is a small refactor of existing code rather than new modeling work; the rest
of this document assumes both entry points exist with the signatures above.
...
the null M-step are circularly shifted. The biases
\(\mathbf B^{\text{null},(b,p)}\) are initialized from
\(\hat{\mathbf B}^{(b)}\) and refit under that frozen state timeline.
...
study); optionally the refit null biases
\(\{\mathbf B^{\text{null},(b,p)}\}_{p=1}^{P}\) for diagnostics; the fitted
...
delayed schedule fired, final state occupancy). Stage~(b) consumes only
these files and never re-runs PRISM-EM.
...
\begin{enumerate}
  ...
  \((Y_{t-1},Y_t)\), and \(\mathbf B\) initialized from
  \(\hat{\mathbf B}^{(b)}\) but refit.
  \item Write \(\hat A^{(b)}\), \(\{A^{\text{null},(b,p)}\}\), \(\hat S^{(b)}\),
  realized block starts, boundary pair indices, block-start retry violations,
  realized shifts, fitted sparsity, and history summary, with a bag-indexed
  filename.
\end{enumerate}
