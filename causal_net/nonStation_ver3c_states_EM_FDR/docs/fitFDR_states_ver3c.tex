...
lag-1 state alignment is \(\hat S^{\text{lock}}_{t-1}\) for pair
\((Y_{t-1},Y_t)\), implemented by \texttt{onehot\_from\_prev\_states}.
...
\((q,\ \pi_{\text{thr}})\): a per-bag per-row quantile level \(q\) and a
...
the bound assumes independent exchangeable bags, whereas overlapping-block
bags are positively correlated, so the reported value should be read as an
approximate stability diagnostic rather than as a guaranteed FDR.
...
\begin{description}
  ...
  \item[\texttt{--bagsTag TAG}] Optional alphanumeric tag appended after
...
The helper script \texttt{big\_fit\_bags.sh} is a concrete multi-bag launch
...
\texttt{daleN100\_2ba29b\_c47b43}. It then defines
\[
\texttt{bagTag="j\$\{SLURM\_JOB\_ID: -4\}"},
\]
so all bags launched inside one SLURM job share the same Stage~(a) extended
name
\[
\texttt{<shortN>\_<bagTag>}.
\]
For each requested bag index, the script runs
\texttt{torchrun --standalone --nnodes=1 --nproc\_per\_node=4} on
\texttt{prism\_EM\_FDR\_Bags\_train3c.py}. With the current loop
\texttt{for bag in 3 4 5}, the expected output files are
\begin{verbatim}
<basePath>/prismFDR/<shortN>_<bagTag>.bag003.prismFDRbag.npz
<basePath>/prismFDR/<shortN>_<bagTag>.bag004.prismFDRbag.npz
<basePath>/prismFDR/<shortN>_<bagTag>.bag005.prismFDRbag.npz
\end{verbatim}
If the commented loop \texttt{for bag in 0 1 2} is used instead, the files
are \texttt{bag000}, \texttt{bag001}, and \texttt{bag002}; \texttt{bag000}
is the chronological reference bag. To aggregate the full set, Stage~(b)
must be called with \texttt{--dataName <shortN>\_<bagTag>} and
...
\begin{description}
  ...
  \texttt{<originalDataName>\_<bagsTag>}. This exact string is the prefix of
