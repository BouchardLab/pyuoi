...
\begin{equation}
  ...
  c_t^{\mathrm{(M\text{-}step)}} = e_{\hat{S}_{t-1}}
  \quad\hbox{for lag pair }(Y_{t-1},Y_t),
\end{equation}
where \(e_m\) is the \(m\)-th standard basis vector. In the current ordinary
EM implementation the one-hot row supplied to the M-step for the pair
\((Y_{t-1},Y_t)\) is the previous-bin decoded state \(\hat S_{t-1}\), matching
the saved metadata \texttt{mstep\_state\_mode=viterbi\_onehot\_prevbin}.
This ensures the M-step fits each state's parameters from cleanly separated
data rather than from soft mixtures.
