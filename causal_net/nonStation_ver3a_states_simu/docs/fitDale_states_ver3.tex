...
\begin{abstract}
This document describes the current \texttt{prism\_EM\_train3.py} implementation
(script header: \texttt{prism\_EM\_train.py}) for fitting a non-stationary
...
\bottomrule
\end{tabular}
\caption{Current defaults in \texttt{prism\_EM\_train3.py}.}
\end{table}
...
Wrapper script \texttt{fitPrismEM.sh} launches distributed training via
\texttt{torchrun} (default 4 GPUs) and sets
\texttt{--time\_range\_sec 0 80} unless overridden.
...
\begin{itemize}
  ...
  \item \texttt{e\_nll\_em}, \texttt{m\_loss\_epoch}, \texttt{m\_nll\_epoch},
        \texttt{m\_l1\_epoch}, \texttt{rho\_epoch}, \texttt{nz\_edges\_epoch},
        \texttt{learning\_rates}: training history.
\end{itemize}
Metadata records all CLI hyperparameters, time-window bins, initialization
dictionaries, and \texttt{mstep\_state\_mode=viterbi\_onehot}.
