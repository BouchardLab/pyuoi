...
$G$, define the in-degree of the target and the out-degree of the
...
\begin{equation}
  r = \frac{
    ...
    k_i^{\mathrm{in}}\,k_j^{\mathrm{out}}
    \;-\;
    \left[|\mathcal{E}|^{-1}\!\displaystyle\sum_{(i,j)\in\mathcal{E}}
    k_i^{\mathrm{in}}\right]
    \left[|\mathcal{E}|^{-1}\!\displaystyle\sum_{(i,j)\in\mathcal{E}}
    k_j^{\mathrm{out}}\right]
  }{
    \sigma_{\mathrm{in}}\;\sigma_{\mathrm{out}}
  },
  \label{eq:assortativity}
\end{equation}
where $\sigma_{\mathrm{in}}$ and $\sigma_{\mathrm{out}}$ are the
standard deviations of $\{k_i^{\mathrm{in}}\}_{(i,j)\in\mathcal{E}}$
and $\{k_j^{\mathrm{out}}\}_{(i,j)\in\mathcal{E}}$ over all edges.
...
% ===============================================================
\section{Method 3: Spectral Dimension from the Random Walk}
\label{sec:spectral_dim}
% ===============================================================

Consider a random walker on the symmetrized graph $\tilde{G}$ with
transition matrix $P = \tilde{D}^{-1}\tilde{G}$, where
$\tilde{D}_{ii}=\sum_j\tilde{G}_{ij}$.  The average return
probability after $t$ steps is
\begin{equation}
  p_{\mathrm{ret}}(t)
  = \frac{1}{N}\operatorname{tr}(P^t)
  = \frac{1}{N}\sum_{k=1}^{N}\mu_k^{\,t},
  \label{eq:return_prob}
\end{equation}
where $1=\mu_1\geq\mu_2\geq\cdots\geq\mu_N$ are the eigenvalues
of~$P$.  For graphs embedded in $d$-dimensional space,
$p_{\mathrm{ret}}(t)\sim t^{-d_s/2}$ at intermediate times, defining
the \emph{spectral dimension}~$d_s$.

Rather than fitting a single slope over an arbitrary window, we
compute the \emph{scale-dependent spectral dimension}
\begin{equation}
  d_s(t)
  = -2\,\frac{d\log p_{\mathrm{ret}}}{d\log t},
  \label{eq:local_ds}
\end{equation}
evaluated via symmetric finite differences at integer~$t$.  We then
extract two scalar diagnostics: the \emph{plateau value}
$\bar{d}_s$, obtained by averaging $d_s(t)$ over the interval where
it exceeds $80\%$ of its maximum, and the \emph{plateau width}~$W$,
the length of that interval.

As $\delta_{\mathrm{ker}}$ increases, $\bar{d}_s$ decreases
monotonically toward~$2$ (the embedding dimension) while $W$ grows,
reflecting clean power-law scaling over a widening range of diffusion
times.  In the Erd\H{o}s--R\'enyi limit ($\delta_{\mathrm{ker}}=0$),
long-range shortcuts cause $d_s(t)$ to rise without settling, and no
plateau forms.  The pair $(\bar{d}_s, W)$ therefore provides a
complementary diagnostic to the combinatorial methods of
Sections~\ref{sec:assortativity} and~\ref{sec:jaccard}.

% ===============================================================
\section{Method 4: Cycle Closure Decay Rate}
\label{sec:cycles}
% ===============================================================
...
% ===============================================================
\section{Method 5: Persistent Homology}
...
For graphs embedded in two dimensions, only $k=0$ (connected
components) and $k=1$ (loops) carry meaningful geometric information;
higher-dimensional features such as enclosed cavities ($k=2$) are not
expected in a planar embedding, and their computation requires
enumerating $(k{+}1)$-cliques, which becomes prohibitively expensive
for large graphs.  We therefore restrict attention to the pair
$(\Pi_0,\Pi_1)$.

For lattice-like graphs ($\delta_{\mathrm{ker}}\geq 2$), the
dimension-$0$ barcode consists of a single long bar and many short
bars, yielding small~$\Pi_0$, while the dimension-$1$ barcode shows
persistent loops reflecting the regular mesh, giving
moderate~$\Pi_1$.  In the Erd\H{o}s--R\'enyi limit
($\delta_{\mathrm{ker}}=0$), random long-range edges create many
short-lived components and loops, inflating~$\Pi_0$ and producing a
cloud of short bars in~$\Pi_1$ with no dominant persistent features.

The pair $(\Pi_0,\Pi_1)$ thus offers a purely topological diagnostic
of the lattice-to-random transition, complementing the combinatorial
and spectral methods of
Sections~\ref{sec:assortativity}--\ref{sec:spectral_dim}.
...
The five diagnostics are collected in
...
\ref{sec:spectral_dim}
  & Spectral dimension & $d_s$ & $\downarrow$
  & no & $O(N^3)$ eigendecomp \\
\ref{sec:cycles}
  & Cycle closure decay & $\xi$ & $\downarrow$
  & no & $O(K\,N^3)$ or eigendecomp \\
\ref{sec:persistent_homology}
  & Persistent homology & $(\Pi_0,\Pi_1)$ & $\Pi_0\!\downarrow,\;
    \Pi_1\!\uparrow$ & optional & $O(n_{\mathrm{simp}}^3)$ reduction \\
\bottomrule
\end{tabular}
\caption{Summary of the five topological diagnostics for the decay
...
Methods~1--4 operate exclusively on the binary adjacency matrix~$G$
...
overlap count edges and shared neighbors; spectral dimension and cycle
closure decay work with powers of~$G$ or its symmetrization, both of
which depend only on which entries are nonzero.

Persistent homology (Method~5) is the only diagnostic that can
...
depending on weight magnitudes that the other four methods ignore.

The five methods probe complementary facets of the same underlying
...
two-hop neighborhood structure.  Spectral dimension (Method~3) probes
the global diffusion geometry.  Cycle closure decay (Method~4)
interpolates between local and global scales through the
parameter~$k$.  Persistent homology (Method~5) captures the
topological skeleton of the graph --- its connected components and
...
$k_{\max}$, and E/I ratio as the target network), compute all five
scalars for each, and fit monotone calibration curves.  Given an
observed graph, the five diagnostics are evaluated and mapped through
the calibration curves to produce five independent estimates of
