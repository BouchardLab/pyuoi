...
def run_method5(A_off, dmd, verb=1):
    import gudhi
    A = np.abs(A_off)
...
if __name__ == "__main__":
    ...
    info4 = run_method4(G, dmd, verb=args.verb)
    info5 = run_method5(A_off, dmd, verb=args.verb)
    ...
    write_data_npz({}, outFF, metaD=outMD)
    print(f"\nSaved: {outFF}")
    print("\n#Summary: dataName, ker_delta, r_assort, jaccard, cycle_decay, hom_k1")
    print("#Values: %s  %.1f  %.6f  %.6f  %.6f  %.6f" % (args.dataName, dmd["placement_ker_delta"], info1["r_assortativity"], info2["mean_jaccard"], info4["cycle_decay"], info5["homology_k1"]))
