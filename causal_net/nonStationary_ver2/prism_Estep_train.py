...
def main():
    ...
    args.truthPath = os.path.join(args.basePath, "truthDale")
    ...
    assert isinstance(spikeMD, dict), "Expected metadata dict in spikes file"
    assert "provenance" in spikeMD, "Missing provenance in spikes metadata"
    ...
    assert "state_model_file" in prov, "Missing state_model_file in provenance"
    truth_name = prov["state_model_file"]

    truthFF = os.path.join(args.truthPath, f"{truth_name}.simTruth.npz")
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for t0_idx in range(1, T_eff, args.chunk_size):
            ...
            for t in range(t0_idx, t1_idx):
                ...
                for _ in range(args.pgd_iter):
                    ...
                    g = g_data + 2.0 * args.lambda2 * (c_t - c_prev)
                    c_t = c_t - args.lr * g
        ...
        if args.verb > 0:
            ...
            print(
                ...
                f"loss={loss_mean:.4e}  elaT={elaT:.1f}s"
            )

    # Decode most probable state and confidence level
    if M == 1:
        S_hat = torch.zeros((T_eff,), dtype=torch.int64, device=c_hat.device)
        S_hat_CL = torch.ones((T_eff,), dtype=torch.float32, device=c_hat.device)
    else:
        top2 = torch.topk(c_hat, k=2, dim=1)
        S_hat = top2.indices[:, 0].to(dtype=torch.int64)
        S_hat_CL = (top2.values[:, 0] - top2.values[:, 1]).to(dtype=torch.float32)
    ...
    print('   ./prism_Estep_eval.py  --basePath $basePath  --dataName %s   -p a b f  \n ' % (out_base))
