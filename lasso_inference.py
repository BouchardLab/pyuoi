...
def InverseLinftyOneRow(sigma,i,mu,maxiter=50,threshold=1e-2):
    ...
    if mu>=mu0:
        ...
        for i in xrange(sigma_tilde.shape[0]):
            sigma_tilde[i,i] = 0
...
def InverseLinfty(sigma,n,resol=1.5,mu=None,maxiter=50,threshold=1e-2,verbose=True):
    p = sigma.shape[0]
    M = np.zeros((p,p))
    beta = np.zeros(p)
    isgiven = True
...
def SSLasso(X,y,alpha=0.05,lambd=None,mu =None,\
    ...
    interval_sizes = stats.norm.ppf(1.-(alpha*1./2))*s_hat*np.sqrt(np.diag(A))*1./np.sqrt(n)
    ...
    p_vals = 2*(1-stats.norm.cdf(np.sqrt(n)*np.abs(unbiased_Lasso)\
                /(s_hat*col_norm[intercept*1:]*np.sqrt(np.diag(A[intercept*1:,intercept*1:])))))
...
#            yhat = np.dot(X[T,:],est_beta)
        ...
        bic = len(T)*np.log(np.dot(rsd,rsd)*1./len(T))+np.log(len(T))*p
