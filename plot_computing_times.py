...
if experiment == 'size':
    ...
    m = [10] * len(n)
    l = [10000] * len(n)
    labels = ['0.4', '0.8', '4', '8', '40', '80', '400', '800', '4000', '8000',
              '40000']
...
save = False
if save:
    figurePath = '/global/project/projectdirs/m2043/BRAINgraph/results/scalability_test/figures/%s/%s' % (
    machine, experiment)
...
for i in range(2):
    for ii in range(2):
        for iii in range(len(n)):
            try:
                if i == 1:
                    fname = 'MPI_BoLBO'
                else:
                    fname = 'BoLBO'
                filename = '%s/%s/%s/%s/%s_ExpI_%i_0.0_%i.0_%i.0_f8.h5' % (
                resultsPath, k[i], experiment, machine, fname, n[iii], l[iii],
                m[iii])
                with h5py.File(filename, 'r') as f:
                    if ii == 0:
                        ...
                        res[i, ii, iii] = f.attrs['loadTime'] + f.attrs[
                            'saveTime']
            except:
...
labels = ['4e2', '8e2', '4e3', '8e3', '4e4', '8e4', '4e5', '8e5', '4e6', '8e6',
          '4e7']

axCB = plt.axes([.45, .3, .4, .04])
axCB.imshow(np.vstack((col, col)), aspect='auto', interpolation='nearest',
...
p1 = ax2.bar(np.arange(1, len(n) + 1), res[1, 1], .35, color='r',
             edgecolor='none')
p2 = ax2.bar(np.arange(1, len(n)), res[1, 0, :-1], .35, color='pink',
             edgecolor='none',
             bottom=res[1, 1, :-1])
...
p1 = ax2.bar(np.arange(1, len(n) + 1) + .37, res[0, 1], .35, color='k',
             edgecolor='none')
p2 = ax2.bar(np.arange(1, len(n) - 2) + .37, res[0, 0, :-3], .35, color='gray',
             edgecolor='none',
             bottom=res[0, 1, :-3])

ax2.bar(np.arange(9, len(n) + 1) + .37, res[0, 0, -3:], .35, color='white',
        edgecolor='gray',
        bottom=res[0, 1, -3:], hatch='//')

plt.xticks(np.arange(1, len(n) + 1) + .36, labels, fontsize=6)
ax2.set_yscale('log')
ax2.legend((p1[0], p2[0]), ('IO', 'compute'), loc='best', frameon=False,
           fontsize=8)
