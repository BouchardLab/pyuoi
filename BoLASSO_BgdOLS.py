...
def main():
    ...
    parser.add_option("--with-admm",action='store_true',\
        dest='with_admm',help="use ADMM solver")
    ...
    BoLASSO_BgdOLS(bgdOpt=options.bgdOpt,nrnd=options.nrnd,initrnd=options.initrnd,\
                    nbootE=options.nbootE,cvlfrct=options.cvlfrct,rndfrct=options.rndfrct,\
...
def BoLASSO_BgdOLS(inputFile,outputFile,bgdOpt=1,nrnd=10,initrnd=10,\
                    ...
                    seed=1234,comm=None,nbootS=48,rndfrctL=.8,with_admm=False,\
                    dtype='f4',nsamp=None,ncov=None,n_minibatch=10):
    ...
    end_loadTime = time.time()-start_loadTime
    ...
    for c in xrange(initrnd):
        ...
        for i in xrange(nMP):
            ...
            if not with_admm:
                ...
                B0[c,:,i] = admm.lasso_admm(X[train],\
        ...
        for c in xrange(nbootE):
            ...
            for i in xrange(nMP):
                ...
                if len(sprt_ids)>0:
                    ...
                    print '%i parameters were selected for lambda: %.4f'%(sprt_ids.sum(),lambL[i])
