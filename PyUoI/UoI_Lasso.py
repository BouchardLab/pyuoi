...
class UoI_Lasso(lm.base.LinearModel, SparseCoefMixin):
    ...
    def fit(self, X, y, seed=None, verbose=False, sample_weight=None):
        ...
        # set up data arrays
        estimates = np.zeros((self.n_boots_est, self.n_lambdas, n_features_), dtype=np.float32)
        ...
        for bootstrap in trange(self.n_boots_est, desc='Model Estimation'):
            ...
            for lamb_idx, lamb in enumerate(lambdas):
                if np.any(self.supports_[lamb_idx]):
                    ...
                    scores[bootstrap, lamb_idx] = -boot_train_split * np.log(rss/boot_train_split) \
                                                    -self.n_features_ * np.log(boot_train_split)
                else:
        ...
        self.scores_ = explained_variance_score(y_true, y_hat)
