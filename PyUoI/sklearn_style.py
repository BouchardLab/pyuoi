...
class UoI_Lasso():
    ...
    def __init__(self, n_lambdas=48, n_randomizations=1,
                 selection_thres_frac=1.0,
                 train_frac_sel=0.8, train_frac_est=0.8,
                 train_frac_overall=0.9,
    ...
    def fit(self, X, y, seed=None, verbose=False):
        ...
        estimates_coarse, explained_variance_coarse = self.lasso_sweep(X, y,
                                                                       lambda_coarse,
                                                                       self.train_frac_sel,
                                                                       self.n_boots_coarse,
                                                                       self.n_minibatch,
        ...
        if verbose: print(
            '(2) Beginning model selection. Exploring penalty region centered at %d.' % lambda_max)
        ...
        estimates_dense, explained_variance_dense = self.lasso_sweep(X, y,
                                                                     lambdas,
                                                                     self.train_frac_sel,
                                                                     self.n_boots_sel,
                                                                     self.n_minibatch,
        ...
        self.coef_ = np.zeros((self.n_randomizations, n_features_),
                              dtype=np.float32)
        self.explained_variance_ = np.zeros(self.n_randomizations,
                                            dtype=np.float32)
        ...
        for randomization in trange(self.n_randomizations, desc='model estimation'):
            ...
            for bootstrap in range(self.n_boots_est):
                ...
                for lamb_idx, lamb in enumerate(lambdas):
                    if np.any(self.supports_[lamb_idx]):
                        ...
                        print(
                            'No variables selected in the support for lambda = %d.' % lamb)

            if verbose: print(
                '(4) Bagging estimates, using bagging option %s.' % self.bagging_options)
            ...
            if self.bagging_options == 1:
                ...
                self.coef_[randomization, :] = np.median(best_estimates,
                                                         axis=0)
                ...
                self.coef_[randomization, :] = np.median(
                    estimates[:, lambda_max_idx, :], 0)
            else:
                raise ValueError(
            ...
            self.explained_variance_[randomization] = explained_variance_score(
                y_true, y_hat)
