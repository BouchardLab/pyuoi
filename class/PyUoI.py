...
class UoI_Lasso():
	...
	----------
	n_lambdas_ : int
		number of L1 penalty values to compare across (effectively sets the hyperparameter sweep)

	n_randomizations_ : int
		number of times to perform the model estimation module
		(default is 1 randomization, more can be used for cross-validation)

	selection_thres_frac : float
		used for soft thresholding in the selection step. normally, UoI-Lasso requires
		regressors to be selected in _all_ bootstraps to be selected for use in the estimation module.
		this requirement can be softened with this variable, by requiring that a regressor
		appear in selection_thres_frac of the bootstraps.

	train_frac_sel : float
		fraction of dataset to be used for training in the selection module.

	train_frac_est : float
		fraction of dataset to be used for training in each bootstrap in the estimation module.
	...
	bagging_options : int
		equal to 1: for each bootstrap sample, find the regularization parameter that gave the best results
		equal to 2: average estimates across bootstraps, and then find the regularization parameter that gives the best results

	n_minibatch : int
		number of minibatches to use in case SGD is used for Lasso (selection) or OLS (estimation).

	use_admm : boolean
		flag indicating whether to use the ADMM algorithm.

	X : (2d) np array of floats, shape = (number of samples) x (number of features)
		design matrix

	y : (1d) np array of floats, shape = (number of samples)
		response vector
	...
	coef_ : np array of floats, shape = (number of randomization) x (number of features)
		contains the UoI-Lasso estimates for each randomization

	explained_variance_ : np array of floats, shape = (number of randomizations)
	...
	def __init__(self, n_lambdas=48, n_randomizations=1, selection_thres_frac=1.0,
	...
	def fit(self, X, y, seed=None, verbose=False):
		...
		estimates_coarse, explained_variance_coarse = self.lasso_sweep(self.X, self.y, lambda_coarse, self.train_frac_sel, self.n_boots_coarse, self.n_minibatch, self.use_admm)
		...
		if verbose: print('(2) Beginning model selection. Exploring penalty region centered at %d.' %lambda_max)
		
		# create the final lambda set based on the coarse sweep
		if self.n_lambdas == 1:
			lambdas = np.array([lambda_max])
		else:
			lambdas = np.linspace(lambda_max - 5 * d_lambda, lambda_max + 5 * d_lambda, self.n_lambdas, dtype=np.float64)
		# run the lasso sweep with new lambda set
		estimates_dense, explained_variance_dense = self.lasso_sweep(self.X, self.y, lambdas, self.train_frac_sel, self.n_boots_sel, self.n_minibatch, self.use_admm)
		...
		if verbose: print('(3) Model selection complete. Beginning model estimation, with %s randomizations and %s bootstraps.' %(self.n_randomizations, self.n_boots_est)) 
		
		# create or overwrite arrays to collect final results (over all randomizations)
		self.coef_ = np.zeros((self.n_randomizations, self.n_features_), dtype=np.float32)
		self.explained_variance_ = np.zeros(self.n_randomizations, dtype=np.float32)
		...
		for randomization in range(self.n_randomizations):
			...
			explained_variance = np.zeros((self.n_boots_est, self.n_lambdas), dtype=np.float32)
			...
			for bootstrap in range(self.n_boots_est):
				...
				for lamb_idx, lamb in enumerate(lambdas):
					if np.any(self.supports_[lamb_idx]):
						...
						explained_variance[bootstrap, lamb_idx] = explained_variance_score(y_true_boot, y_hat_boot)
					else:
						# if no variables were selected, throw a message 
						print('No variables selected in the support for lambda = %d.' %lamb)
			
			if verbose: print('(4) Bagging estimates, using bagging option %s.' %self.bagging_options)
			
			# perform the bagging for each randomization
			if self.bagging_options == 1:
				...
				self.coef_[randomization, :] = np.median(best_estimates, axis=0)
				...
				self.coef_[randomization, :] = np.median(estimates[:, lambda_max_idx, :], 0)
			else:
				raise ValueError('Bagging option %d is not available.' %self.bagging_options)
			# finally, see how the bagged estimates perform on the test set
			y_hat = np.dot(X_test, self.coef_[randomization, :])
			y_true = y_test - y_test.mean()
			# calculate and store performance of the final UoI_Lasso estimator over test set
			self.explained_variance_[randomization] = explained_variance_score(y_true, y_hat)

		if verbose: print("---> UoI Lasso complete.")
