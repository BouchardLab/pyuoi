...
def main():
    ...
    Model_Data_Create(mdlW=options.mdlW, v1=options.v1, v2=options.v2,
                      v3=options.v3, mdlsz=options.mdlsz, store=options.store,
                      path=options.path, seed=options.seed,
                      saveAs=options.saveAs, dtype=options.dtype)


def Model_Data_Create(mdlW, v1, v2, v3, mdlsz=100,
                      seed=np.random.randint(9999), store=False,
                      path=os.getcwd(), saveAs='hdf5', dtype='f4'):
    """
    Model_Data_Create
    -----------------
    ...
    if mdlW == 'Gaus':
        ...
        lp = np.linspace(mn, mx, 6)
        for i in range(5):
            lpW = np.exp(np.linspace(lp[i], lp[i + 1], np.floor(mdlsz / 10.)))[
                  :, np.newaxis]
            if i == 0:
                W = np.vstack((-lpW, lpW))
            else:
    ...
    if store:
        ...
        mdlW, mdlsz, str(v1 * 100), str(v2 * 100), str(v3 * 100), dtype)
        if saveAs == 'hdf5':
